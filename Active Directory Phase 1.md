...
Install-ADDSForest -DomainName "lab.local" -DomainNetBiosName "LAB" -InstallDns:$true -SafeModeAdministratorPassword (ConvertTo-SecureString "P@ssw0rd123!" -AsPlainText -Force) -Force:$true
```
...
$pw = (ConvertTo-SecureString "Lab@12345!" -AsPlainText -Force)

New-ADUser -Name "John Doe"   -SamAccountName jdoe    -UserPrincipalName jdoe@lab.local    -Path "OU=IT,DC=lab,DC=local"       -AccountPassword $pw -Enabled $true
New-ADUser -Name "Mary Smith" -SamAccountName msmith  -UserPrincipalName msmith@lab.local  -Path "OU=HR,DC=lab,DC=local"       -AccountPassword $pw -Enabled $true
New-ADUser -Name "Bob Wilson" -SamAccountName bwilson -UserPrincipalName bwilson@lab.local -Path "OU=Finance,DC=lab,DC=local"  -AccountPassword $pw -Enabled $true
New-ADUser -Name "Tom Lee"    -SamAccountName tlee    -UserPrincipalName tlee@lab.local    -Path "OU=Helpdesk,DC=lab,DC=local" -AccountPassword $pw -Enabled $true

Add-ADGroupMember -Identity "IT-Admins"     -Members jdoe
Add-ADGroupMember -Identity "HR-Staff"      -Members msmith
Add-ADGroupMember -Identity "Finance-Staff" -Members bwilson
Add-ADGroupMember -Identity "Helpdesk-T1"   -Members tlee
```
